...
1. **License**: not uniform, corrected 2026-09-13 after a prior version of this line wrongly said "MIT by default with one exception" — a live check of every linked repo's actual LICENSE file found 29 of 65 are AGPL-3.0, not 7 or 8. The real split (see suite README's License section, and `estate/manifest.json` → `repos_license_audit` for the authoritative per-repo list): specs, Python services, Rust crates, per-language SDKs, and the narrower MCP servers (`mcp-decision-intelligence`, `mcp-permission-broker`, `mcp-reliability-toolkit`) default to **MIT**. GitHub Actions, CLI/crawler tooling, `mcp-kinetic-gain` and its sibling AGPL MCP tools, and the Cloud Identity/Platform/FinOps/Threat-Detection governance lane default to **AGPL-3.0**. The Operator Console is **Apache-2.0**, on its own. When creating a new repo, match whichever tier it actually belongs to rather than assuming MIT is the default — check the README's License table first.
